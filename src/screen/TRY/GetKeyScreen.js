  ...
  return (
    ...
    <View style={styles.container}>
      ...
      {myList.map((liste, index) => (
        <View key={index}>
          <Text style={styles.listName}>{liste.listeAdi}</Text>
          <View style={styles.itemsContainer}>
            {liste.listeMaddeleri.map((madde, idx) => (
              <Text key={idx} style={styles.item}>{madde.sira}. {madde.aciklama}</Text>
            ))}
          </View>
        </View>
      ))}
      <Button title="MyList'i Yenile" onPress={() => {
        AsyncStorage.getItem('myList').then(jsonValue => {
          setMyList(JSON.parse(jsonValue));
        }).catch(error => console.error('myList yenilenirken hata oluştu:', error));
      }} />
    </View>
    </ScrollView>
  );
